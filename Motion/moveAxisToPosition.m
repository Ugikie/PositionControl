function [] = moveAxisToPosition(MI4190,desiredPos,AZCurrPos,loadBarProgress,loadBar,measApp,incrementSize,anglesRemaining)
...
if (incrementSize == 'N')
  measApp.StatusTable.Data = {AZCurrPos;desiredPos;AZCurrVel;'N/A'};
else
...
if (AZCurrVel == 0.0000)
    ...
    measApp.writeConsoleLine('[%s] Moving Axis (AZ) from position: %.2f, to desired position: %.2d',datestr(now,'HH:MM:SS.FFF'), AZCurrPos, desiredPos);
    ...
    dots(4);
    verifyIfInPosition(MI4190,desiredPos,0.6,loadBarProgress,loadBar,measApp,anglesRemaining,incrementSize);
    
    if (measApp.wantToStop) return; end
    
else
    cprintf('err','[%s][WARNING] Axis is in motion and cannot be moved!\n',datestr(now,'HH:MM:SS.FFF'));
    measApp.writeConsoleLine('[%s][WARNING] Axis is in motion and cannot be moved!\n',datestr(now,'HH:MM:SS.FFF'));
